...
export async function extractMonthData(
  ctx: Ctx,
  month: ElementHandle<Element>,
) {
  try {
...
export async function getMonthContainers(ctx: Ctx) {
  ...
  if (months.length !== 2)
    throw "Couldn't find exactly 2 month calendar elements";

  return months as [ElementHandle<Element>, ElementHandle<Element>];
}

export async function switchMonth(
  ctx: Ctx,
  month: ElementHandle<Element>,
  monthLabel: string,
) {
  ...
  await ctx.page.waitForSelector(".dropdown__toggle");
  ...
  await ctx.page.waitForSelector(".dropdown-item__label");
  ...
  await ctx.page.waitForSelector(
    `.dropdown__toggle-text::-p-text("${monthLabel}")`,
  );
}
...
export async function removeCookies(ctx: Ctx) {
  try {
    const cookies = await ctx.page.waitForSelector(
